...
// ---- for testing inference ----
    ...
    //#include "wav.h"
        ...
        public:
            ...
            std::string c_str_sec()
            {
                //return std::format("timestamp {:08d}, {:08d}", start, end) in sec;
                return format("{start:%08d s , end:%08d s}", start / 16000, end / 16000);
            };
        private:
        ...
        public:
            ...
            const float get_speech_prob()
            {
                return ort_outputs[0].GetTensorMutableData<float>()[0];
            }
        ...
        public:
            ...
            //  {
                //init_onnx_model(ModelPath);     // Initiate ONNX model
            /**/

            /* Construction with.c model */
            VadIterator(
                int Sample_rate = 16000, int windows_frame_size = 32,
                float Threshold = 0.5, int min_silence_duration_ms = 0,
                int speech_pad_ms = 32, int min_speech_duration_ms = 32,
                float max_speech_duration_s = std::numeric_limits<float>::infinity())
            {
                init_onnx_model();     // Initiate ONNX model
                /**/
    ...
    // rest of testing code inside Paint fuction
// --------------------------------
...
void AudioPluginAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    // ---- for testing inference ----
        std::vector<float> input_wav(3000);
        VadIterator vad;
        vad.process(input_wav);
        //float speech_prob = vad.get_speech_prob();
        g.drawFittedText(std::to_string(vad.get_speech_prob()), getLocalBounds(), juce::Justification::centred, 1);
	// --------------------------------
}
